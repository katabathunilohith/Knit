...
import type { DocumentStats, CompactionResult } from '../types/index.js';
...
export class CollaborativeRoom {
  ...
  public roomAwareness: RoomAwareness;
  public createdAt = Date.now();
  public lastCompactedAt: number | null = null;
  ...
  private seedInitialContent() {
    ...
    if (xmlFragment.length === 0) {
      this.ydoc.transact(() => {
        ...
        introText.insert(
          0,
          'Welcome to the zero-data-loss collaborative rich-text editor powered by TipTap, Yjs CRDTs, and local-first IndexedDB persistence.'
        );
...
export class DocumentManager {
  ...
  public getOrCreateRoom(docName: string): CollaborativeRoom {
  ...
  public getRoom(docName: string): CollaborativeRoom | undefined {
